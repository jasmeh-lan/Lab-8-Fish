...
Missing_Fish <- Fish |>
  summarise(across(everything(), ~ sum(is.na(.)))) |> # Creating a summary of the total missing values for each of the variables/columns
  pivot_longer(everything(), names_to = "variable", values_to = "missing_count") |> # I had to use CHATGPT to help me turn the multiple rows into columns to create a cleaner table
  filter(missing_count > 0) # Filter out the variables that have no missing data
  
Missing_Fish
```

> There are 1796 observations that have missing values and they are all in the fish variable of weight.
...
```{r}
#| label: Visual Data Prepare

Visual <- Fish |>
  mutate(missing = as.integer(if_any(everything(), is.na))) |>
  group_by(year, section, trip) |>
  summarise(num_miss = sum(missing), .groups = "drop" ) |>
  mutate(year_trip = paste(year, trip)) # I noticed that there are just two different sections and two different trips so I could either chose year-section or year-trip and facet by one of the two variables in graph (Used ChatGPT for paste function)
```
...
Visual |>
  ggplot(aes(x = num_miss, y = fct_reorder(year_trip, num_miss))) + 
  geom_point() +
  labs(x = "Number of Missing Values", 
       y = "Records of Year with Number of Trip", 
       title = "Missing Values of Fish Data Across Different Years, Sections, and Trips") +
  theme_bw() +
  facet_wrap(~section)
```
...
> The mistake is that in the weight part of the code, in the denominator, you used length instead of weight for the min portion. There should be a weight variable there instead of length.
...
rescale_01 <- function(x){(x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE))} # I used PA-9 Assignment as an example to help me figure out how to make a function
```
...
# Original Graph
plot1 <- ggplot(Fish, aes(x = length)) + 
  geom_histogram(binwidth = 45) +
  labs(x = "Original Values of Fish Length (mm)") +
  scale_y_continuous(limits = c(0,4000))

# Scaled Graph
plot2 <- ggplot(data = Fish, 
       aes(x = length_scaled)) +
  geom_histogram() +
  labs(x = "Rescale Values of Fish Length") +
  scale_y_continuous(limits = c(0,4000))

plot1 + plot2 #Using Patchwork to show side by side comparisons
```
